...
$root = $PSScriptRoot
# Tests pass -OutputRoot to build into a throwaway temp directory instead of
# dist\, so a test run can never clobber the analyzer-gated release artifact.
# Subexpression form required - (if ...) as an inline argument is a runtime
# error on PS 5.1.
$dist = $(if ($OutputRoot) { $OutputRoot } else { Join-Path $root 'dist' })
...
if ($Pdq) {
    ...
    foreach ($tool in @($registryData.Tools)) {
        ...
        $g.Add('# Exit codes: 0 Success | 1 Failed | 2 Warning | 3 Skipped | 4 Refused')
        $g.Add('# PDQ Deploy treats non-zero as failure - configure 2 and 3 as additional')
        $g.Add('# success codes on this package if this tool can legitimately return them.')
        $g.Add('')
        $g.Add('[CmdletBinding()]')
        $g.Add('param(')
        # Only a Disruptive tool actually refuses without -Force (exit 4). Stamping
        # this note on every script regardless of Risk would mislead a technician
        # running a ReadOnly or Modifies tool into thinking -Force is needed here.
        if ($tool.Risk -eq 'Disruptive') {
            $g.Add('    [switch]$Force,      # REQUIRED for this tool (Disruptive) - refuses with exit 4 otherwise')
        } else {
        ...
        # Cast to [bool] rather than interpolating the registry value directly.
        # A non-boolean value (e.g. a stray string like 'yes') would otherwise
        # get written as a bareword `$yes`, which parses cleanly, passes both
        # gates, and evaluates to $null on the endpoint - silently becoming
        # Refused (exit 4) with nothing in the build to flag it.
        $g.Add(("        RequiresAdmin = `${0}" -f ([bool]$tool.RequiresAdmin)))
        $g.Add(("        SilentCapable = `${0}" -f ([bool]$tool.SilentCapable)))
        $g.Add(("        PdqDeployable = `${0}" -f ([bool]$tool.PdqDeployable)))
